...
import { createAddress, updateAddress, getSubPrefectures } from "@/fetcher/api-fetcher";
  ...
  useEffect(() => {
    if (isOpen) {
      const fetchSubPrefectures = async () => {
        ...
        try {
          const response = await getSubPrefectures({ limit: 1000 });
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        {trigger || (
          ...
          >
            {isEdit ? "Modifier" : "Ajouter une nouvelle adresse"}
          </Button>
        )}
      </DialogTrigger>
      <DialogContent className="sm:max-w-[425px] bg-white dark:bg-cyan-950 border-cyan-200 dark:border-cyan-800 fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
        <DialogHeader>
          <DialogTitle className="text-cyan-700 dark:text-cyan-300">{isEdit ? "Modifier une adresse" : "Ajouter une adresse"}</DialogTitle>
        </DialogHeader>
