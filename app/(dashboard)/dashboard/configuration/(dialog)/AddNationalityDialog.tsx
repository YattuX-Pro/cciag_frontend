...
import { createNationality, updateNationality } from "@/fetcher/api-fetcher";
...
export default function AddNationalityDialog({
  onSuccess,
  trigger,
  nationality,
  isEdit,
  ...
  useEffect(() => {
    if (nationality && isOpen) {
      setValue("name", nationality.name);
    }
  }, [nationality, isOpen, setValue]);

  const onSubmit = async (data: Nationality) => {
    try {
      setIsLoading(true);
      if (isEdit && nationality) {
        ...
        toast({
          title: "Succès",
          description: "La nationalité a été modifiée avec succès.",
        });
      } else {
        ...
        toast({
          title: "Succès",
          description: "La nationalité a été ajoutée avec succès.",
        });
      }
      setIsOpen(false);
      reset();
      ...
      toast({
        title: "Erreur",
        description: isEdit
          ? "Une erreur est survenue lors de la modification de la nationalité."
          : "Une erreur est survenue lors de l'ajout de la nationalité.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        {trigger || (
          ...
          >
            {isEdit ? "Modifier" : "Ajouter une nouvelle nationalité"}
          </Button>
        )}
      </DialogTrigger>
      <DialogContent className="sm:max-w-[425px] bg-white dark:bg-cyan-950 border-cyan-200 dark:border-cyan-800 fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
        <DialogHeader>
          <DialogTitle className="text-cyan-700 dark:text-cyan-300">{isEdit ? "Modifier une nationalité" : "Ajouter une nationalité"}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          >
            <div className="space-y-2">
              <Label className="text-cyan-700 dark:text-cyan-300">Nom de la nationalité</Label>
              <Input 
                {...register("name", { required: "Le nom de la nationalité est requis" })}
