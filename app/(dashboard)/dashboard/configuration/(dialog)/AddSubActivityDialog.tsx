...
import { createSubActivity, updateSubActivity } from "@/fetcher/api-fetcher";
...
export default function AddSubActivityDialog({
  onSuccess,
  trigger,
  subActivity,
  isEdit,
  ...
  // Set form values when subActivity changes or dialog opens
  useEffect(() => {
    if (subActivity && isOpen) {
      setValue("name", subActivity.name);
    }
  }, [subActivity, isOpen, setValue]);
  ...
  const onSubmit = async (data: SubActivity) => {
    try {
      ...
      if (isEdit && subActivity) {
        ...
        toast({
          title: "Succès",
          description: "La sous-activité a été modifiée avec succès.",
        });
      } else {
        ...
        toast({
          title: "Succès",
          description: "La sous-activité a été ajoutée avec succès.",
        });
      }
      setIsOpen(false);
      reset();
      ...
      toast({
        title: "Erreur",
        description: isEdit
          ? "Une erreur est survenue lors de la modification de la sous-activité."
          : "Une erreur est survenue lors de l'ajout de la sous-activité.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        {trigger || (
          ...
          >
            {isEdit ? "Modifier" : "Ajouter une nouvelle sous-activité"}
          </Button>
        )}
      </DialogTrigger>
      <DialogContent className="sm:max-w-[425px] bg-white dark:bg-cyan-950 border-cyan-200 dark:border-cyan-800 fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
        <DialogHeader>
          <DialogTitle className="text-cyan-700 dark:text-cyan-300">{isEdit ? "Modifier une sous-activité" : "Ajouter une sous-activité"}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          >
            <div className="space-y-2">
              <Label className="text-cyan-700 dark:text-cyan-300">Nom de la sous-activité</Label>
              <Input 
                {...register("name", { required: "Le nom de la sous-activité est requis" })}
