...
import { createActivity } from "@/fetcher/api-fetcher";
...
export default function AddSectorDialog({
  ...
  const onSubmit = async (data: Activity) => {
    try {
      ...
      toast({
        title: "Succès",
        description: "Le secteur d'activité a été ajouté avec succès",
      });
      setIsOpen(false);
      reset();
      onSuccess?.();
      ...
      toast({
        title: "Erreur",
        description: "Une erreur est survenue lors de l'ajout du secteur",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[425px] bg-white dark:bg-cyan-950 border-cyan-200 dark:border-cyan-800 fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
        <DialogHeader>
          <DialogTitle className="text-cyan-700 dark:text-cyan-300">Ajouter un secteur d'activité</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          >
            <div className="space-y-2">
              <Label className="text-cyan-700 dark:text-cyan-300">Nom du secteur</Label>
              <Input 
                {...register("name", { required: "Le nom du secteur est requis" })}
